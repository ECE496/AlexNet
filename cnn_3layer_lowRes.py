...
for i in range(1, 32121):
	img = cv2.imread("/groups/vdkagrp/fer2013/images/outfile" + str(i) + ".jpg")
	img_gray = img[:, :, 0]
	X.append(img_gray)
# first = True
...
y_processed = (np.arange(num_class) == Y[:, None]).astype(np.float32)
...
def baseline_model_saved():
    ...
    json_file = open('model_3layer_2_2_pool.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    model = model_from_json(loaded_model_json)
    #load weights from h5 file
    model.load_weights("model_3layer_2_2_pool.h5")
    model.compile(optimizer='adam', loss='binary_crossentropy', metrics=[categorical_accuracy])
    return model
...
if(is_model_saved==False ):
    ...
    with open("model_3layer_2_2_pool.json", "w+") as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    model.save_weights("model_3layer_2_2_pool.h5")
...
y_test2 = [ np.argmax(item) for item in y_processed]
...
# 	prediction = model.predict_classes(img_gray)
...
img = cv2.imread("/groups/vdkagrp/capstone/PYTHONSCRIPT/disgust.jpg")
...
img_gray = img_gray.reshape(1, 48, 48, 1)
prediction = model.predict_classes(img_gray)
print("Nikola is " + str(label_map[prediction[0]]))